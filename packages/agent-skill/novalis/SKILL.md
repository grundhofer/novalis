...
| `search <query> [--tag T] [--folder F] [--limit 50] [--snippets] [--all-files] [--regex] [--case-sensitive]` | on-demand scan over notes, or with `--all-files` over every regular file in the vault; `cloudOnlySkipped` and `notUtf8Skipped` count what was not searched |
