...
**Amended by:** ADR-0019 (2026-09-15) — `board.json` is `{format, name, columns, updated}` plus an optional `order`, a fractional-index key written only when the user drags a board in the tree; boards without one follow the keyed ones by name. A board field, not a card field: the card-field rule above stands.
