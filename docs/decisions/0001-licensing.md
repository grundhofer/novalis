...
## Carried over 2026-09-05

Copied verbatim from the original repository (`docs/decisions/0001-licensing.md`,
accepted 2026-08-09) into the rewrite's scaffold; nothing above this rule was
edited. The decision stands for the rewrite: AGPL-3.0-only with both section-7
permissions, `COMMERCIAL-LICENSE.md`, and the inbound grant in `CONTRIBUTING.md`.
Two facts changed underneath it and are recorded here instead of by editing the
text: the rewrite ships no git sync, so libgit2 and LibXDiff are no longer linked
and the LGPL argument above is historical; and the rewrite has no plugin API, so
permission #1 is dormant but kept ("harmless to keep", PLAN.md §4.5). The
corresponding-source obligation now reads as in PLAN.md §11.6: the git archive
plus lockfiles, attached to the same GitHub Release (`docs/RELEASING.md`).

**Owner approval:** 2026-09-05 — "section 4 please make your recommedations"
(docs/DECISIONS.md, "Owner answers of 2026-09-05"), adopting the PLAN.md §4.5
row "License: AGPL-3.0-only + COMMERCIAL-LICENSE, ADR-0001 copied verbatim".
