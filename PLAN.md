...
- **External-change state machine (one spec shared by editor, board store, watcher, CLI, sync):**
  ...
  2. Watcher batch marks the path changed:
     ...
     - buffer dirty → banner *„Auf der Festplatte geändert"* / "Changed on disk" with **Neu laden / Meine behalten / Als Konfliktkopie speichern**; if the disk diff is exactly a link rewrite from a rename, re-apply it to the buffer instead of raising the banner.
      - the path is gone (deleted by another device, the Finder, the CLI) and still gone when looked at — a delete-and-recreate save is an ordinary change → buffer clean: the tab closes; buffer dirty: autosave stops and a banner *„Auf der Festplatte gelöscht — dein Text ist noch hier"* offers **Schließen / Meine behalten** (writes the buffer as the file again). A save that finds the file gone before the watcher reports it takes the same banner, not a conflict copy (feature-gaps A17, 2026-09-22).
...
| Data | `json json5 jsonc` · `yaml yml` · `toml` · `xml xsl xsd plist svg` · `csv tsv log` · `diff patch` · `bib` · `ini properties cfg env conf` | data (mono, numbers, indent detected, 2 by default) | JSON, YAML, TOML, XML, diff, Properties files; `*nginx*.conf` Nginx; `csv tsv log bib conf` plain — csv/tsv are not a table editor; behind the eye glyph / ⌘E a CSV or TSV is a read-only table and an SVG its picture (ADR-0025) |
...
| View (ADR-0015/0016/0023/0024/0025) | `pdf png jpg jpeg gif webp heic heif avif` · `epub cbz` · `docx` | viewer, read-only | none for images (`<img>` with the app's zoom: bar, pinch or `Ctrl`-wheel, click for fit ↔ 1:1, ADR-0025); PDF through pdf.js on a canvas with the app's own page/zoom bar and selectable text, the document's outline in the EPUB reader's contents popover and the page keys of §7.4 (ADR-0025), no annotation; EPUB through the app's own reader on the core's zip primitive (`read_packed`), one chapter at a time into a Shadow-DOM host through an allow-list, contents popover and chapter bar; CBZ on the same primitive, one page as an image with the next one prefetched, a page bar and the image zoom; DOCX through `mammoth` into the same Shadow-DOM host, capped at 15 MB, with Word's `~$…` lock files left unlisted; no annotation and nothing fetched |
...
Apple standard untouched (`Cmd-Z/Shift-Cmd-Z`, `Cmd-F/G`, `Cmd-S`, `Cmd-N/O/W/Q`, `Cmd-B/I` apply `**`/`_` in Markdown, `Ctrl-Cmd-F` full screen; `Cmd-,` unbound in v1). Editor: `Cmd-P` quick-open · `Cmd-Shift-P` palette · `Ctrl-G` goto line · `Cmd-D` next occurrence · `Cmd-Shift-L` all occurrences · `Cmd-Alt-F` replace · `Cmd-Shift-F` vault search · `Cmd-/` comment (code) · `Ctrl-Shift-↑/↓` add cursor · `Cmd-Ctrl-↑/↓` move line · `Shift-Cmd-D` duplicate line · `Ctrl-Shift-K` delete line · tabs `Shift-Cmd-[`/`]`, `Cmd-1…9`, `Shift-Cmd-T` · navigation `Cmd-[`/`]` · `Cmd-K` link · `Cmd-Enter` toggle checkbox · `Cmd-\` sidebar · `Cmd-Shift-B` board · `Cmd-=`/`-`/`0` font size · `Cmd-J` Today's Note (ADR-0026). Tree: `Enter` rename, `Cmd-Delete` trash, `Shift-Cmd-N` new folder. PDF pane (ADR-0025): `←`/`→`, `PageUp`/`PageDown` turn the page, `Home`/`End` first and last.

### 7.5 Spellcheck — settled by Spike B (2026-09-05)

tauri-apps/tauri#7705 reproduces on macOS 26.6: out of the box a WKWebView shows no red underlines, though the right-click menu already offers the correct German suggestions. The cause is one user-defaults key, not a missing WebKit feature: with `WebContinuousSpellCheckingEnabled` set, the underlines appear (measured three ways — sending `toggleContinuousSpellChecking:`, writing the key before launch, and a control run without it). **v1 therefore ships real underlines**: the shell seeds that key from the `spellcheck` setting at startup and the existing Edit ▸ Spelling item toggles both. The `NSSpellChecker`-decoration fallback is dropped from v1.1. Report: `docs/spikes/2026-09-05-spike-b-spellcheck.md`. Measured in the app on 2026-09-22 (feature-gaps A2): the key does the same inside the app's own web view, but WebKit reads it once, when the first web view is made — a write after that (from `setup`, or from another process) showed nothing until the next launch. So the shell writes it in-process (CoreFoundation `CFPreferencesSetAppValue`, `apps/desktop/src-tauri/src/spelling.rs`) and only then builds the window (`"create": false` in `tauri.conf.json`); the Edit ▸ Spelling item writes it again and takes effect live, for text typed from then on. In the built app (not in `just dev`) WebKit also left the first editor of a launch unchecked; the editor sets its `spellcheck` attribute once more at the first key typed in a session, which makes WebKit look again (`editor/Editor.tsx`).
...
| `cat <note>…` | `--body`, `--frontmatter`, `--lines A:B`, `--materialize [--timeout 30s]` | text, or `{items:[{path,title,linkTarget,frontmatter,body,sha256,utf8,links:[{target,form,line,resolvedPath}]}]}` | read; cloud-only → exit 8 unless `--materialize`; the exact path of a non-note text file reads that file, a binary one is exit 2 (ADR-0036) |
...
| `journal` | `--date YYYY-MM-DD\|today\|yesterday` (default today, local), `--append <text\|->`, `--materialize` | `{path,stem,linkTarget,existing,sha256}` (+`appended`) | create `journal/<date>.md` empty when missing (like the app's Today row), then append; idempotent; `sha256` is after the call (ADR-0027) |
...
| `search <query>` | `--tag`, `--folder`, `--limit 50`, `--snippets`, `--all-files`, `--regex`, `--case-sensitive` (ADR-0036) | `{items:[{path,line,snippet}],truncated,cloudOnlySkipped:N,notUtf8Skipped:N}` | on-demand scan over notes; `--all-files` every regular file, unfiltered (ADR-0022) |
...
| `board ls` / `board show <b>` / `board new <b> [--name N]` / `board columns <b> --set <json>` | | boards, columns, cards | boards store; `new` creates `boards/<b>/board.json` with no columns, exit 4 if it exists (ADR-0031) |
