...
Apple standard untouched (`Cmd-Z/Shift-Cmd-Z`, `Cmd-F/G`, `Cmd-S`, `Cmd-N/O/W/Q`, `Cmd-B/I` apply `**`/`_` in Markdown, `Ctrl-Cmd-F` full screen; `Cmd-,` unbound in v1). Editor: `Cmd-P` quick-open · `Cmd-Shift-P` palette · `Ctrl-G` goto line · `Cmd-D` next occurrence · `Cmd-Shift-L` all occurrences · `Cmd-Alt-F` replace · `Cmd-Shift-F` vault search · `Cmd-/` comment (code) · `Ctrl-Shift-↑/↓` add cursor · `Cmd-Ctrl-↑/↓` move line · `Shift-Cmd-D` duplicate line · `Ctrl-Shift-K` delete line · tabs `Shift-Cmd-[`/`]`, `Cmd-1…9`, `Shift-Cmd-T` · navigation `Cmd-[`/`]` · `Cmd-K` link · `Cmd-Enter` toggle checkbox · `Cmd-\` sidebar · `Cmd-Shift-B` board · `Cmd-=`/`-`/`0` font size. Tree: `Enter` rename, `Cmd-Delete` trash, `Shift-Cmd-N` new folder. PDF pane (ADR-0025): `←`/`→`, `PageUp`/`PageDown` turn the page, `Home`/`End` first and last.
